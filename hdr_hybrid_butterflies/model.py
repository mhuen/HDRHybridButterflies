...
class WingCNNClasifier(CNNClasifier):
    def __init__(
        self,
        num_classes: int,
        image_size: Tuple[int, int],
        ...
        dtype: str = "float32",
        verbose: bool = True,
        name: str = "WingCNNClassifier",
    ) -> None:
        """Initialize the WingCNNClassifier",.
        ...
        verbose : bool, optional
            Whether to print verbose output, by default True.
        name : str, optional
            Name of the model, by default "WingCNNClassifier",".
        """
        tf.keras.Model.__init__(self, dtype=dtype)
        ...
        # create CNNs for upper and lower wing segments
        self.cnn_upper = tfs.ConvNdLayers(
            input_shape=[-1, image_size[0], image_size[1], 3],
            padding_list="SAME",
            hex_zero_out_list=False,
            hex_num_rotations_list=1,
            float_precision=dtype,
            name=name + "__cnn_upper",
            verbose=verbose,
            **cnn_config,
        )

        self.cnn_lower = tfs.ConvNdLayers(
            input_shape=[-1, image_size[0], image_size[1], 3],
            padding_list="SAME",
            hex_zero_out_list=False,
            hex_num_rotations_list=1,
            float_precision=dtype,
            name=name + "__cnn_lower",
            verbose=verbose,
            **cnn_config,
        )
        ...
        self.fc_layers = tfs.FCLayers(
            input_shape=[
                -1,
                cnn_config["num_filters_list"][-1] * 2,
            ],
            weights_list=None,
            biases_list=None,
            max_out_size_list=None,
        ...
        # explicitly add these variables to the module
        # variables created in sub-module are not found otherwise..
        self._cnn_upper_vars = self.cnn_upper.variables
        self._cnn_lower_vars = self.cnn_lower.variables
        self._fc_vars = self.fc_layers.variables
    ...
    def call(self, inputs: tf.Tensor, is_training: bool = False) -> tf.Tensor:
        ...
        inputs : tf.Tensor
            Input tensor to the CNN. This is the game state.
            Shape: (batch_size, 4, image_size[0], image_size[1], 3)
        is_training : bool, optional
            Whether the model is training, by default False.
        ...
        inputs_upper = tf.reshape(
            inputs[:, :2], [-1, self.image_size[0], self.image_size[1], 3]
        )
        inputs_lower = tf.reshape(
            inputs[:, 2:], [-1, self.image_size[0], self.image_size[1], 3]
        )

        # apply cnn
        cnn_out_upper = self.cnn_upper(inputs_upper, is_training=is_training)[
            -1
        ]
        cnn_out_lower = self.cnn_lower(inputs_lower, is_training=is_training)[
            -1
        ]

        # Shape: (batch_size, 2, x_dim, y_dim, num_filters)
        cnn_out_upper = tf.reshape(
            cnn_out_upper, [-1, 2] + cnn_out_upper.shape[1:].as_list()
        )
        cnn_out_lower = tf.reshape(
            cnn_out_lower, [-1, 2] + cnn_out_lower.shape[1:].as_list()
        )

        # Shape: (batch_size, x_dim, y_dim, num_filters)
        cnn_out_upper = tf.reduce_mean(cnn_out_upper, axis=1)
        cnn_out_lower = tf.reduce_mean(cnn_out_lower, axis=1)
        cnn_out = tf.concat([cnn_out_upper, cnn_out_lower], axis=-1)

        if is_training and self.verbose:
            print("cnn_out_upper", cnn_out_upper.shape)
            print("cnn_out_lower", cnn_out_lower.shape)
...
class CNNSegmenter(CNNClasifier):
    def __init__(
        self,
        num_classes: int,
        image_size: Tuple[int, int],
        cnn_config: dict[str:Any] = {
            "filter_size_list": [[10, 10]] * 20,
            "num_filters_list": [12] * 20,
            "pooling_type_list": None,
            "pooling_strides_list": [[1, 2, 2, 1]] * 20,
            "pooling_ksize_list": [[1, 2, 2, 1]] * 20,
            "strides_list": [1, 1, 1, 1],
            "dilation_rate_list": [[1, 1, 1, 1], [1, 3, 3, 1]] * 10,
            "use_dropout_list": False,
            "use_batch_normalisation_list": False,
            "activation_list": "elu",
            "use_residual_list": True,
        ...
        dtype: str = "float32",
        verbose: bool = True,
        name: str = "CNNSegmenter",
    ) -> None:
        """Initialize the CNNSegmenter",.
        ...
        max_guesses : int, optional
            Maximum number of guesses to make, by default 6.
        cnn_config : dict[str, Any], optional
            Configuration for the CNN layers. These are
            passed on to the `tfscripts.layers.ConvNdLayers` class.
        dtype : str, optional
            Data type to use for the model, by default "float32".
        verbose : bool, optional
        ...
        num_filters_list = cnn_config["num_filters_list"]
        num_filters_list[-1] = num_classes
        cnn_config["num_filters_list"] = num_filters_list
    ...
    def call(self, inputs: tf.Tensor, is_training: bool = False) -> tf.Tensor:
        ...
        tf.Tensor
            Logits for the model.
            Shape: (batch_size, image_size[0], image_size[1], num_classes)
        """
        ...
        # apply cnn
        logits = self.cnn(inputs, is_training=is_training)[-1]
        if is_training and self.verbose:
