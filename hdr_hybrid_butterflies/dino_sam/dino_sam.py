...
def polygon_to_mask(
    ...
    polygon : List[Tuple[int, int]]
        List of (x, y) coordinates representing the
        vertices of the polygon.
    image_shape : Tuple[int, int]
        Shape of the image (height, width) for the mask.
...
def detect(
    ...
    """Apply zero-shot object detection using Grounding DINO.

    Use Grounding DINO to detect a set of labels in
    an image in a zero-shot fashion.
    """
...
def segment(
    ...
    """Segment objects in an image using Segment Anything (SAM).

    Use Segment Anything (SAM) to generate masks given an
    image + a set of bounding boxes.
    """
    device = "cuda" if torch.cuda.is_available() else "cpu"
    segmenter_id = (
        segmenter_id if segmenter_id is not None else "facebook/sam-vit-base"
    )
